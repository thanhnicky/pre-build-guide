...
import { n as nestLists, L as LIST_NEST_MODE_HTML, c as isPortableTextToolkitList, a as isPortableTextListItemBlock, d as isPortableTextToolkitSpan, i as isPortableTextBlock, e as isPortableTextToolkitTextNode, s as spanToPlainText, b as buildMarksTree } from "./portabletext__toolkit.mjs";
const defaultLists = {
  number: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("ol", { children }),
  bullet: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { children })
}, DefaultListItem = ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children }), link = ({ children, value }) => /* @__PURE__ */ jsxRuntimeExports.jsx("a", {
  href: value?.href,
  children
...
function PortableText({ value: input, components: componentOverrides, listNestingMode, onMissingComponent: missingComponentHandler = printWarning }) {
  ...
  let nested = nestLists(blocks, listNestingMode || LIST_NEST_MODE_HTML), components = reactExports.useMemo(() => componentOverrides ? mergeComponents(defaultComponents, componentOverrides) : defaultComponents, [componentOverrides]), renderNode = reactExports.useMemo(() => getNodeRenderer(components, handleMissingComponent), [components, handleMissingComponent]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: nested.map((node, index) => renderNode({
    node,
    index,
    isInline: false,
    renderNode
